...
use crate::queries::{InviteUserMutation,invite_user_mutation};
#[cfg(test)]
use wasm_bindgen_test::*;

pub async fn invite_poet(email:String,token:Option<String>)
...
async fn test_invite_poet() {
    ...
    let resp =
        invite_poet(
            "test_email@test_email.test_email".into(),None)
            .await.unwrap();
    match resp {
        GraphQlResp::Data(_) => {},
