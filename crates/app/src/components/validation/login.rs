    ...
    let req = {
        ...
        use_async::<_, (), String>(async move {
            ...
            match if admin_login {
                let resp = post_graphql::<SuperAdminLoginMutation>(
                ...
                (
                    resp.data
                        .as_ref()
                        .map(|data| data.super_admin_login.clone()),
                    resp.errors.clone(),
                )
            } else {
                let resp =
                    post_graphql::<LoginMutation>(login_mutation::Variables { email, pass }, None)
                        .await
                        .map_err(|err| format!("{:?}", err))?;
                (
                    resp.data.as_ref().map(|data| data.login.clone()),
                    resp.errors.clone(),
                )
            } {
                ...
                (Some(data), None) => {
                    ...
                    msg_context
                        .dispatch(new_green_msg_with_std_duration("Login Successful.".into()));
                ...
                (_, errors) => {
                    msg_context.dispatch(new_red_msg_with_std_duration(
                        map_graphql_errors_to_string(&errors),
                    ));
