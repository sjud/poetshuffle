...
use entity::{invitations::Model as Invitation, prelude::Invitations};
...
impl PublishMutation {
    async fn accept_invitation(
        ...
        if !invitation.fufilled {
            if let Some(login) = find_login_by_email(db, &invitation.invitee_email).await? {
                ...
                if let Some(password) = password {
                    ...
                    .await?;
                    Ok(format!(
                        "Account created with role {:?}, please log in\n\
        using the email that received the invitation and your password.",
                        invitation.user_role
                    ))
                } else {
            ...
            Ok(format!(
                "This invitation has been completed and your account was updated to {:?}\
             status\n
             at {:?},\n please log in to to continue as your new role.",
                invitation.user_role,
                invitation.fufilled_ts.ok_or(Error::new(
                    "\
...
mod test {
    ...
    async fn test_accept_invitation() {
        ...
        assert_eq!(
            ...
            "{acceptInvitation: \"NEEDS_PASSWORD\"}"
        );
    }
}
