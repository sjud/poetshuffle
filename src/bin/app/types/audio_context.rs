...
impl Reducible for AudioOptions {
    ...
    fn reduce(self: Rc<Self>, action: Self::Action) -> Rc<Self> {
        match action {
            AudioActions::SwitchVisibility => Rc::new(
                Self{is_visible:!self.is_visible,..(*self).clone() }),
            AudioActions::SwitchPlaying =>
                Rc::new(Self{is_playing:!self.is_playing,..(*self).clone() }),
